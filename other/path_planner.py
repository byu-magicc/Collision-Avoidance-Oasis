...
class PathPlanner:
    ...
    def __init__(self, goal_pos, time_forward=5., max_velocity=23., ts=0.2) -> None:
        ...
        self.num_control_points = time_forward/ts
        self.ts = ts
    ...
    def update(self, own_pos, intruder_pdfs) -> BSpline.Curve: # expect a nested list of intruder pdfs for each timestep into the future
        ...
        def objective_function(x, goalPosition=(20,20)):
        ...
        bounds = [(0,20) for i in range(len(initial_x))]
        res = minimize(objective_function, initial_x, method='SLSQP', bounds=bounds, constraints=[max_velocity_constraint, avoidance_constraint])
        cp = []
